...
- Who may edit comes from one guard check, `Auth::guard($guard)->check()`, with `$guard` from the config (default `web`). A guard the application does not define is treated as "nobody may edit" rather than throwing, so a typo costs the editing and not the page. The check is made when the component is drawn, in `openModal()` and in `save()`; the last two answer 403. `translationKey` and `html` are locked, so set them on the tag, never with `->set()`. With the default guard `web` every logged in user may edit, so point the package at a guard for editors. There is no per user or per key permission; if you need one, extend the component, override the protected `isAuthorized()` (not `render()`), and register your class as `inline-translation`. A public action you add yourself must start with `$this->authorizeEditing()`.
